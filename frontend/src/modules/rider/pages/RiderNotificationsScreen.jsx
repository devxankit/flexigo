        ...
        <AnimatePresence mode='popLayout'>
          {notifications.length > 0 ? (
            notifications.map((ntf, idx) => (
              ...
              >
                <GlassCard 
                  className={`p-5 flex items-start gap-4 border relative ${
                    isDark ? 'border-white/5 hover:border-emerald-500/20' : 'border-slate-200 hover:border-emerald-500/30'
                  } ${!ntf.isRead ? (isDark ? 'bg-emerald-500/5' : 'bg-emerald-50') : ''}`}
                  onClick={() => markAsRead(ntf.id)}
                >
                  <div className={`w-10 h-10 rounded-2xl flex items-center justify-center shrink-0 border ${
                    ntf.type === 'payment' ? 'bg-emerald-500/10 border-emerald-500/20 text-emerald-500' :
